...
class ReplayMemory:
    ...
    def __init__(self, size, num_actions, discount_factor=0.95):#7):
        """
    ...
    def update_all_q_values(self):
        ...
        for k in reversed(range(self.num_used-1)):
            ...
            if end_life or end_episode:
                ...
                #print('death action',action,'reward',reward)
                action_value = reward
    ...
    def prepare_sampling_prob(self, batch_size=128):
        ...
        #print('idx',idx)
        #print('np.where(idx)',np.where(idx))
        self.idx_err_lo = np.squeeze(np.where(idx))
        #print('self.idxerrlo',self.idx_err_lo)
        # Create an index of the estimation errors that are high.
        #print('np.logical_not(idx)',np.logical_not(idx))
        #print('np.where',np.where(np.logical_not(idx)))
        self.idx_err_hi = np.squeeze(np.where(np.logical_not(idx)))
        #print('self.idxerrhi',self.idx_err_hi)
...
class NeuralNetwork:
    ...
    def optimize(self, min_epochs=1.0, max_epochs=10,
        ...
        for i in range(max_iterations):
            ...
            #print('statebatch',state_batch,type(state_batch))
            #print('qvalues',q_values_batch,type(q_values_batch))
...
class Agent:
    ...
    def __init__(self, env_name, training, render=False, use_logging=True):
        ...
        self.env = gym.make(env_name,frameskip=5,render_mode='human')
        ...
        if self.training:
            ...
            self.learning_rate_control = LinearControlSignal(start_value=1e-4,#3,
                                                             end_value=1e-5,
                                                             num_iterations=5e6)
